...
export default function TopBar() {
  const { personaId } = useStore()
  const initial = personaId ? personaId.slice(-1).toUpperCase() : 'V'
  ...
  return (
    <header className="h-16 flex items-center gap-4 px-4 bg-black text-white">
      ...
      <div className="flex-1 flex items-center justify-center gap-2 max-w-2xl mx-auto">
        <button
          className="w-12 h-12 rounded-full bg-spotify-highlight grid place-items-center hover:bg-[#333] transition shrink-0"
          aria-label="Home"
        >
      ...
      <div className="hidden sm:flex items-center gap-3 shrink-0">
        ...
        <div className="w-8 h-8 rounded-full bg-[#7b46b0] grid place-items-center text-white text-sm font-bold">
          {initial}
        </div>
      </div>
    </header>
